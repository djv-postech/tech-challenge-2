...
class CadastroDePedidoTest {
  @InjectMocks private CadastroDePedido cadastroDePedido;
  @Mock private PedidoRepository pedidoRepository;

  @DisplayName("Test - Deve atualizar status do pedido")
  @Test
  public void dadoPedido_QuandoCadastrarPedido_EntaoDeveRetornarPedidoCadastrado() {
    // Dado
    Produto produto =
        new Produto(
    ...
    given(pedidoRepository.cadastrarPedido(pedido)).willReturn(pedido);

    // Quando
    Pedido pedidoCadastrado = cadastroDePedido.cadastrarPedido(pedido);

    // Entao
    Assertions.assertThat(pedidoCadastrado).isNotNull();
    Assertions.assertThat(pedidoCadastrado.getNumeroPedido()).isEqualTo(pedido.getNumeroPedido());
  }
}
